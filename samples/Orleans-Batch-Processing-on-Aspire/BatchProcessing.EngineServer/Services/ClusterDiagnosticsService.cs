...
namespace BatchProcessing.EngineServer.Services;

internal class ClusterDiagnosticsService(IGrainFactory grainFactory, ISiloMetadataCache siloMetadataCache)
{
    ...
    public async ValueTask<CallGraph> GetGrainCallFrequencies()
    {
        ...
        foreach (var silo in silos)
        {
            ...
            {
                if (activation.GrainId.IsSystemTarget())
                    continue;
        ...
        {
            if (edge.TargetGrain.IsSystemTarget() || edge.SourceGrain.IsSystemTarget())
                continue;
        ...
        {
            var metadata = siloMetadataCache.GetSiloMetadata(hostId).Metadata;
            var details = _hostDetails[hostId];
            hostIds[key] = new(hostId.ToString(), details.ActivationCount, metadata);
        }
...
public record struct HostNode(string Name, int ActivationCount, IDictionary<string, string> Metadata);
